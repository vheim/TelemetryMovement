#TOP OF CODE----
### ====================================================================================================
### Project:    Movement ecology of shortfin mako sharks off Mexico
### Analysis:   Visualising Utilisation Distributions (UDs), i.e. spatial probablity densities
### Script:     ~2025_mexico_makos\R\script7_Mexico_makos_visualise_spaceUse_rasters_UDs.R
### Author:     Vital Heim
### Version:    1.0
### ====================================================================================================
...
## Input data
rasterloc <- file.path("/",YOUR_IP, "Science","Projects_current", "2025_Mexico_makos","Data_input", "Movement", "UD_visualisation")
crsloc <- file.path("/",YOUR_IP, "Science","Projects_current", "2025_Mexico_makos","Data_input", "Movement", "UD_visualisation")
...
bathyloc <- file.path("/",YOUR_IP, "Science", "Data_raw", "Bathymetry_maps_GMRT", "GMRTv4_4_0_20251222topo_Mexico.grd")

## Output data
saveloc <- file.path("/",YOUR_IP, "Science","Projects_current", "2025_Mexico_makos","Data_output", "UD_visualisation")
...
### ....................................................................................................
### [B] Prepare your data for plotting at GROUP-LEVEL ----
### ....................................................................................................

# B1: import needed data ----

## UD raster
mov_raster <- stars::read_stars(file.path(rasterloc, "All_Rasters_Scaled_Weighted_UDScaled.asc")) # should not have to change this
### define projection of your raster
CRSdef <- readRDS(file.path(crsloc, "CRS.Rds")) # should not have to change this
...
bathyR <- clamp(bathyR, upper = 5, useValues = F) # this is really just an aesthethic thing, needs to be adjsuted if you print a legend that goes between 0 to -8000 meters

## Extent of the raster
# bbox(bathyR)
# min        max
# s1 -135.720703 -102.62109
# s2    6.227934   38.25889
...
### World EEZ
eez_world <- sf::st_read(dsn = file.path(misc_shapefileloc, "World", "World_EEZ","World_EEZ_v12_20231025","eez_v12.shp"), quiet = TRUE) # read in worldmap
...
### ....................................................................................................
### [C] Visualise your data using ggplot2 at GROUP-LEVEL ----
### ....................................................................................................
...
# bbox_ud_3857 <- st_bbox(ud_surface |> sf::st_transform(3857)) # Get the bounding box (spatial extent), change to your needed epsg
bbox_ud_32612 <- st_bbox(ud_surface |> sf::st_transform(32612)) # Get the bounding box (spatial extent), change to your needed epsg

### Extract min/max longitude and latitude
min_lon <- bbox_ud_32612["xmin"]
max_lon <- bbox_ud_32612["xmax"]
min_lat  <- bbox_ud_32612["ymin"]
max_lat  <- bbox_ud_32612["ymax"]
### Print the results
min_lon
max_lon
min_lat
...
g <- ggplot() +
  ...
  tidyterra::geom_spatraster(data = bathyterra |> terra::project("EPSG:32612"),
                             maxcell = 1e7) + # Increase max cells for higher resolution, default is 5e5
  ...
  # plot the UD surface
  stars::geom_stars(data = ud_surface |> sf::st_transform(32612), inherit.aes = FALSE, # choose your epsg code accordinlgy (here EPSG:3857 is for WGS 84 / Pseudo-Mercator -- Spherical Mercator, Google Maps, OpenStreetMap, Bing, ArcGIS, ESRI)
  ...
  ggplot2::geom_sf(data = stars::st_contour(x = mov_raster,
                                            contour_lines = TRUE,
                                            breaks = max(mov_raster[[1]],na.rm = TRUE) * 0.05) |> sf::st_transform(32612), # define 95 % contour and adjust crs accordingly
                  fill = NA,
                  inherit.aes = FALSE,
                  color = color95,
  ...
  ggplot2::geom_sf(data = stars::st_contour(x = mov_raster,
                                            contour_lines = TRUE,
                                            breaks = max(mov_raster[[1]],na.rm = TRUE) * 0.5) |> sf::st_transform(32612), # define 95 % contour and adjust crs accordingly
                   fill = NA,
                   inherit.aes = FALSE,
                   color = color50,
  ...
  ggplot2::geom_sf(data = world |> sf::st_transform(32612), fill = "gray90", color = "black", size = 0.5) +

  # Plot additional shapefiles if needed (e.g. for small islands)
  ## World EEZs
  # ggplot2::geom_sf(data = eez_world |> sf::st_transform(32612), fill = NA, color = "black", size = 0.5) +

  # Define your plot limits
  coord_sf(xlim = c(min_lon+00100, max_lon-00100), ylim = c(min_lat+00100, max_lat-00100)) + 
...
## save your map
ggsave(file.path(saveloc, "UD_map_Mexixo_makos_group-level.tiff"), plot = g, width = 21, height = 21, units = "cm", dpi = 300)
...
ind_rasters <- list.files(path = rasterloc, 
                           pattern = "^X.*\\.asc$", 
                           full.names = TRUE)
...
valid_input <- FALSE
while (!valid_input) {
  cat("\033[38;5;208mEnter your choice (1 - DYNAMIC or 2 - FIXED extent): \033[0m")
...
for (i in seq_along(ind_rasters)) {
  ...
  if (bbox_option == "fixed") {
    ...
    bbox_ud_32612 <- st_bbox(ud_surface_i |> sf::st_transform(32612))
    min_lon <- bbox_ud_32612["xmin"]
    max_lon <- bbox_ud_32612["xmax"]
    min_lat <- bbox_ud_32612["ymin"]
    max_lat <- bbox_ud_32612["ymax"]
  ...
  p <- ggplot() +
    ...
    # UD surface
    stars::geom_stars(data = ud_surface_i |> sf::st_transform(32612), 
                      inherit.aes = FALSE,
                      color = NA) +
    ...
    ggplot2::scale_linetype_manual(
      name = "Space use",
      values = c("General use" = "dashed", "Core use" = "solid"),
    ...
    # World shapefile
    ggplot2::geom_sf(data = world |> sf::st_transform(32612), 
  ...
  output_filename <- file.path(saveloc, 
...
## BELOW: implementation with terra package and todo's
### ....................................................................................................
### [tbc] Visualise your data using base R,s stars and terra  ----
### ....................................................................................................
